...
const Card = memo(
  forwardRef(
    (
      ...
      const cardClasses = [styles.card, variantClass, sizeClass, className]
        .filter(Boolean)
        .join(' ');
      ...
      return (
        <div ref={ref} className={cardClasses} style={cardStyles} {...props}>
          {children}
        </div>
      );
    }
...
const CardHeader = memo(
  ({ children, hasActions = false, className = '', ...props }) => {
    const headerClasses = [
      styles.header,
...
const CardActions = memo(
  ({ children, position = 'bottom', className = '', ...props }) => {
